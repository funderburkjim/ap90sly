...
def check_bold4(items):
 # Check that any bold {@--n@} is followed by {@--<n+1>@} or {@--Comp.@}
 # or {@1@}
 nitems = len(items)
 for i,item in enumerate(items):
  kind,val = item
  ...
  if val1 == '{@1@}':
   continue
  m1 = re.search(r'{@--([0-9]*?)@}',val1)
  nval1 = int(m1.group(1))
  if nval1 != (nval + 1):
   kind = 'BOLD?a'
   items[i] = (kind,val)
   kind1 = 'BOLD?b'
...
def check_bold4a(items):
 # is first bold item eithe {@1@} or {@Comp.@} ?
 if len(items) == 0:
  return
 item = items[0]
 kind,val = item
...
def out_method1a_err(entry,bolds):
 ...
 for iline,line in enumerate(lines):
  lnum = lnum1 + iline + 1
  outarr.append('%s %s' %(lnum,line))
 outarr.append(';')
 for out in outarr:
  ferr.write(out+'\n')
 Ap90Parser.nparse_err = Ap90Parser.nparse_err + 1
 
def prev_out_method1a_err(entry,bolds):
 outarr = []
 outarr.append('; ' +entry.metaline)
 ibold = 0
...
def out_method1a(entry,results):
 ...
 try:
  ...
  print('error in bold4',entry.metaline)
 check_bold4a(bolds)
 outarr.append(entry.metaline)
 for item in bolds:
  kind,val = item
